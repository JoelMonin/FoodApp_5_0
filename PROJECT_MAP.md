...
- `src/ui/cartPicker.js` : Sélecteur « envoyer une recette vers la liste de courses » (LOT 014 §A, extrait de `js/app.js`). Pré-coche ce qui manque, laisse corriger chaque ligne, puis fusionne avec l'inventaire. Détient l'état PRIVÉ du sélecteur. **Deux couplages INJECTÉS** (`registerCartPickerHooks`) plutôt qu'importés : `openModal`/`closeModal` ne sont pas de simples helpers. Ils étaient TROIS — `buildEmojiEditSuggestions` s'importe directement depuis `src/ui/emojiModal.js` dès que celle-ci est sortie. C'est le nœud annoncé par la phase découverte, détaillé dans l'en-tête du module.
